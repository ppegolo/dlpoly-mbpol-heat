...
#include "dl_params.inc"
      ...
      dimension redmass(mxcons)
      dimension dens(mxsvdw),dens0(mxsvdw),cell0(9)
...
c     calculate centre of mass
      ...
      iatm0 = (natms*idnode)/mxnode + 1
      iatm1 = (natms*(idnode+1))/mxnode
...
c
c     first guess at thermostat variable
...
c     advance position by leapfrog
        ...
        xxx(i) = uxx(i) + tstep*(vxx(i) +
     x    chipnew*((xxx(i)+uxx(i))*0.5d0 - xcmo))
        yyy(i) = uyy(i) + tstep*(vyy(i) +
     x    chipnew*((yyy(i)+uyy(i))*0.5d0 - ycmo))
        zzz(i) = uzz(i) + tstep*(vzz(i) +
     x    chipnew*((zzz(i)+uzz(i))*0.5d0 - zcmo))
...
c     advance velocity by leapfrog
        ...
        gvxx(ig) = gvx1(jg) + tstep*(qn0(jg)/gmass(id) -(chit0+chip0)*
     x       0.5d0*(gvxx(ig)+gvx1(jg)))
        gvyy(ig) = gvy1(jg) + tstep*(qn1(jg)/gmass(id) -(chit0+chip0)*
     x       0.5d0*(gvyy(ig)+gvy1(jg)))
        gvzz(ig) = gvz1(jg) + tstep*(qn2(jg)/gmass(id) -(chit0+chip0)*
     x       0.5d0*(gvzz(ig)+gvz1(jg)))
c     
c     advance position by leapfrog

        gcmx(ig) = gcmx1(jg) + tstep*(gvxx(ig) +
     x    chipnew*((gcmx1(jg)+gcmx(ig))*0.5d0 - xcmo))
        gcmy(ig) = gcmy1(jg) + tstep*(gvyy(ig) +
     x    chipnew*((gcmy1(jg)+gcmy(ig))*0.5d0 - ycmo))
        gcmz(ig) = gcmz1(jg) + tstep*(gvzz(ig) +
     x    chipnew*((gcmz1(jg)+gcmz(ig))*0.5d0 - zcmo))

c
c     centre of mass velocities at half-step
        
        vxt = 0.5d0*(gvx1(jg) + gvxx(ig))
        vyt = 0.5d0*(gvy1(jg) + gvyy(ig))
        vzt = 0.5d0*(gvz1(jg) + gvzz(ig))
c     
c     translational kinetic energy 
