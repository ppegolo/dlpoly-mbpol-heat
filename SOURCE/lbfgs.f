...
C Sandeep:  Check RMS and MAX Forces to terminate optimization
C  RMS Force = sqrt((\sum_atomsI FIX^2+FIY^2+FIZ^2)/N)
...
C     ****************
C
      SUBROUTINE LBFGS(ATMNAM,OUTPUT_FREQ,OUTPUT_INDEX,OUTPUT_RANK,
     x       MAXSTEP,N,M,X,F,G,DIAGCO,DIAG,IPRINT,EPS,XTOL,W,IFLAG)
C

      INTEGER OUTPUT_INDEX,OUTPUT_RANK,MAXSTEP,ATOMS,OUTPUT_FREQ
...
C                        not positive).
C added for max number of steps 
C 
C              IFLAG=-4  Exit if the number of optimization steps exceeds
C                        maxstep given in 'CONTROL' file (default is 10000)
C
C
C    ON THE DRIVER:
C
...
C     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C
      DOUBLE PRECISION GTOL,ONE,ZERO,GNORM,GMAX,DDOT,STP1,FTOL,STPMIN,
...
C
      ...
      OPEN(UNIT=28422,FILE='OPT_COORDINATES.XYZ',ACTION='WRITE')
C
C     INITIALIZE
C     ----------
C
...
C
 ...
 50   W(ISPT+I)= -G(I)*DIAG(I)
      GNORM= DSQRT(DDOT(N,G,1,G,1)/real(N))
c check in case of errors later on whether N should be in divisor or not 
C added
      GMAX= MAXVAL(G)
      STP1= ONE/GNORM
C
...
C     
      ...
      MAXFEV= 20
C
      IF(IPRINT(1).GE.0) CALL LB1(OUTPUT_INDEX,OUTPUT_RANK,IPRINT,ITER,NFUN,
     *                     GNORM,GMAX,N,M,X,F,G,STP,FINISH)
C
C    --------------------
...
C  WRITING COORDINATES
      IF(MOD(ITER,OUTPUT_FREQ)==0) THEN
         WRITE(28422,*) N/3
         WRITE(28422,*) 'geometry optimization step =',ITER
         do ATOMS=1,N/3
            write(28422,"(a8,3F15.5)") ATMNAM(ATOMS),X(3*ATOMS-2),
     x     X(3*ATOMS-1),X(3*ATOMS) 
         enddo
      ENDIF
...
C added
      ...
      IF (GMAX .LE. EPS) FINISH=.TRUE.
C      IF (GNORM .LE. EPS) FINISH=.TRUE.
C      IF (GNORM/XNORM .LE. EPS) FINISH=.TRUE.
C
      IF(IPRINT(1).GE.0) CALL LB1(OUTPUT_INDEX,OUTPUT_RANK,IPRINT,ITER,
     *          NFUN,GNORM,GMAX,N,M,X,F,G,STP,FINISH)
      IF (FINISH) THEN
         IFLAG=0
C writing coordinates at final step 
         WRITE(28422,*) N/3
         WRITE(28422,*) 'final coordinates'
         do ATOMS=1,N/3
            write(28422,"(a8,3F15.5)") ATMNAM(ATOMS),X(3*ATOMS-2),
     x          X(3*ATOMS-1),X(3*ATOMS) 
         enddo
...
C
 ...
 197  IFLAG= -4
      IF(LP.GT.0.AND.OUTPUT_RANK==0) WRITE(LP,241)
C
C     FORMATS
C     -------
C
 ...
 240  FORMAT(/' IFLAG= -3',/' IMPROPER INPUT PARAMETERS (N OR M',
     .       ' ARE NOT POSITIVE)')
 241  FORMAT(/' IFLAG= -4',/' OPTIMIZATION STEPS EXCEEDED. INCREASE '
     .        ' MAXSTEP IN CONTROL FILE TO OPTIMIZE GEOMETRY FURTHER')
...
C
C
      SUBROUTINE LB1(OUTPUT_INDEX,OUTPUT_RANK,IPRINT,ITER,NFUN,
     *                     GNORM,GMAX,N,M,X,F,G,STP,FINISH)
C
C     -------------------------------------------------------------
...
C     -------------------------------------------------------------
C
      INTEGER OUTPUT_INDEX,OUTPUT_RANK
      INTEGER IPRINT(2),ITER,NFUN,LP,MP,N,M
      DOUBLE PRECISION X(N),G(N),F,GNORM,GMAX,STP,GTOL,STPMIN,STPMAX
      LOGICAL FINISH
      COMMON /LB3/MP,LP,GTOL,STPMIN,STPMAX
C
C added
      ...
      IF(OUTPUT_RANK==0) then
       IF (ITER.EQ.0)THEN
           WRITE(MP,10)
           WRITE(MP,20) N/3,M             ! N/3 is total number of atoms
           WRITE(MP,30)F,GNORM,GMAX
...
C
 10   FORMAT('       ')
 20   FORMAT('  No. of real atoms=',I5,'   L-BFGS vector length=',I2,
     .       /,  
     .       /,  'INITIAL VALUES OF')
C 20   FORMAT('  N=',I5,'   NUMBER OF CORRECTIONS=',I2,
 30   FORMAT(' PE= ',1PD10.3,'   GNORM= ',1PD10.3,'   GMAX= ',1PD10.3)
 40   FORMAT(' VECTOR X= ')
 50   FORMAT(6(2X,1PD10.3))
 60   FORMAT(' GRADIENT VECTOR G= ')
 70   FORMAT(/'   Iter ',4X,'PE',8X,'GNORM',7X,'GMAX'/)
C 70   FORMAT(/'   I   NFN',4X,'PE',8X,'GNORM',7X,'GMAX'/)
 80   FORMAT(1(I4,1X),f15.3,3X,3(1PD10.3,2X))
C 80   FORMAT(1(I4,1X),3X,3(1PD10.3,2X))
