...
c     U  proportional to  ( 1/r + ar  + b )
c     such that  dU(rcut)/dr = 0  => a =  1/(rcut)**2
c     and         U(rcut)    = 0  => b = -2/(rcut)
c     
...
c     set cutoff condition for pair forces
      ...
      rcsq=rcut**2
c
c     set force and potential shifting parameters
...
c     calculate potential energy
              ...
              engcpe=engcpe+chgprd*(1.d0/rrr+alpha*rrr+beta)
c     
c     calculate the virial

              egamma=chgprd*(1.d0/rsq - alpha)/rrr
