...
c     
c     shake iterations are here!
c
c     stress tensor for shake
      do i = 1,9
        stres2(i) = 0.d0
      enddo
      vircon = 0.d0
...
c     advance velocity by leapfrog
        ...
        tmp = tstep/gmass(id)
        gvxx(ig) = gvx1(jg) + tmp*qn0(jg)
        gvyy(ig) = gvy1(jg) + tmp*qn1(jg)
        gvzz(ig) = gvz1(jg) + tmp*qn2(jg)
c     
c     advance position by leapfrog
