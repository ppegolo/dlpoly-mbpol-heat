...
! PP_: begin new loop over atoms to store the atomic virial -----28-03-2018---------------------------------------------
                  if (iflag.eq.1) then
                    ...
                    do j = iatm0, iatm1
                      ...
                      smu1i=ckc(i)-dipdotk*skr(i)
                      smu2i=cks(i)+dipdotk*ckr(i)
                      smu1smu2i=ckc(i)*(ckcs-2*skdmuskrs)+
     x                          dipdotk*skr(i)*skdmuskrs+
     x                          cks(i)*(ckss+2*skdmuckrs)+
     x                          dipdotk*ckr(i)*skdmuckrs

              strs1k = scalefactor*((akk-akv*kvec(1)*kvec(1))
     x        *smu1smu2i-2*akk*kvec(1)*dmuskrsx*smu1i+2*akk*
     x        kvec(1)*dmuckrsx*smu2i)

              strs2k = scalefactor*((-akv*kvec(1)*kvec(2))
     x        *smu1smu2i-2*akk*kvec(1)*dmuskrsy*smu1i+2*akk*
     x        kvec(1)*dmuckrsy*smu2i)

              strs3k = scalefactor*((-akv*kvec(1)*kvec(3))
     x        *smu1smu2i-2*akk*kvec(1)*dmuskrsz*smu1i+2*akk*
     x        kvec(1)*dmuckrsz*smu2i)

              strs4k = scalefactor*((-akv*kvec(2)*kvec(1))
     x        *smu1smu2i-2*akk*kvec(2)*dmuskrsx*smu1i+2*akk*
     x        kvec(2)*dmuckrsx*smu2i)

              strs5k = scalefactor*((akk-akv*kvec(2)*kvec(2))
     x        *smu1smu2i-2*akk*kvec(2)*dmuskrsy*smu1i+2*akk*
     x        kvec(2)*dmuckrsy*smu2i)

              strs6k = scalefactor*((-akv*kvec(2)*kvec(3))
     x        *smu1smu2i-2*akk*kvec(2)*dmuskrsz*smu1i+2*akk*
     x        kvec(2)*dmuckrsz*smu2i)

              strs7k = scalefactor*((-akv*kvec(3)*kvec(1))
     x        *smu1smu2i-2*akk*kvec(3)*dmuskrsx*smu1i+2*akk*
     x        kvec(3)*dmuckrsx*smu2i)

              strs8k = scalefactor*((-akv*kvec(3)*kvec(2))
     x        *smu1smu2i-2*akk*kvec(3)*dmuskrsy*smu1i+2*akk*
     x        kvec(3)*dmuckrsy*smu2i)

              strs9k = scalefactor*((akk-akv*kvec(3)*kvec(3))
     x        *smu1smu2i-2*akk*kvec(3)*dmuskrsz*smu1i+2*akk*
     x        kvec(3)*dmuckrsz*smu2i)
                      ! PP_: 13-04-2018: j should be the correct atomic index
