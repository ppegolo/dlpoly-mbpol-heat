     ...
     x  set_vel,iseed,cmd_run,cmd_nve,gamcmd,trpmd_run,
     ...
     x  LangThermoType,numS_gle,omega0,nofix_com,
     x  gle_Aunit,gle_Cunit,gle_Afile,gle_Cfile, omega_cell, tau_cell,
     x  CV_Ql,rmin_ql,rmax_ql,nlist_cutoff,nColVar,ev_pos,
     x  ev_kappa,serveraddr)

      use multibead, only: bead_suffix,ring_size
...
#include "fftw3.f"
      ...
      logical ltscal,lzeql,loptim,ltraj,lfcap,lgofr,lpolar,lthole,lpgr,
     x  lpres,safe,lnsq,lzden,lewald,lspme,lhke,lcp,ldpts,lopt2,lopt,
     x  lpartem,lttm,lttm3,lmsite,lfd,lacs,lads,laspc,lcavity,!,lshake
     x  lmbpol,lmbnrg
...
! PIMD/CMD
      logical :: pimd_npt
      logical :: cmd_run, cmd_nve, set_vel
      logical :: restart_run
      logical :: restart_dipole
      logical :: restart_cmd
      logical :: trpmd_run
      logical :: w_bead
      logical :: hod_in_h2o, hod_in_d2o
      logical :: d2o_in_h2o, h2o_in_d2o
      integer :: nmol_mix, molmix_nskip
...
! Langevin 
      ...
      real*8 :: omega0, omega_cell, tau_cell 
      logical :: fix_com
...
! CV
      integer :: nColVar
      logical :: CV_Ql 
      real*8 :: rmin_ql,rmax_ql,nlist_cutoff,ev_pos(*),ev_kappa(*)
...
! Morale: Langevin 
      LangThermoType = -1   ! type of thermostat: 1-Lang on NM, 2-GLE, 3-PI+GLE
                            ! 4-pile_npt 5-piglet, 6-piglet-npt     
      numS_gle = -1        ! number of additional momenta in GLE
      omega0 = 100.0d0       ! thermostat frequency of normal mode 
      omega_cell = 100.0d0       ! thermostat frequency of cell 
      tau_cell = 0.10d0       ! thermostat frequency of normal mode 
...
c     record is commented out
        ...
        elseif(record(1:6).eq.'plumed') then
          tf_plumed = intstr(record,80,idum)

          tf_plumed=1

          if(tf_plumed.eq.1) then
              lplumed=.true.
            else
              lplumed=.false.
          endif   
          ...
          if(idnode.eq.0) then
            if(lplumed) then
               write(nrite,
     x      "(/,1x,'PLUMED is ON ',3x,i10)") tf_plumed
             else 
               write(nrite,*) "*********************************"
               write(nrite,*) "PLUMED IS NOT activated, CONTINUING
     x             WITH NORMAL MD SIMULATION"
               write(nrite,*) "*********************************"
            endif
...
c     restart control
     ...
     x        "(/,1x,'scaled restart requested')")
        ...
        elseif(record(1:7).eq.'geomopt') then

          lopt = .true.
          if(idnode.eq.0) write(nrite,*)'geometry optimization 
     x           using L-BFGS algorithm'


c      in vibanal 'lopt' is activated to skip dynamic part 
        elseif(record(1:7).eq.'vibanal') then

          lopt = .true.     
          lvib = .true.   
          if(idnode.eq.0) write(nrite,*)'vibrational analysis'
...
c          nstraj = intstr(record,80,idum)
c          ilen = 80 - idum
c          record(1:ilen) = record(idum:80)
c          istraj = intstr(record,ilen,idum)
c          istraj = max(istraj,1)
c          jlen = ilen - idum
c          record(1:jlen) = record(idum:ilen)
c          keytrj = intstr(record,jlen,idum)

          nstraj=intstr(record(starts(2):ends(2)),80,idum)
          istraj=intstr(record(starts(3):ends(3)),80,idum)
          keytrj=intstr(record(starts(4):ends(4)),80,idum)
          keybin=intstr(record(starts(5):ends(5)),80,idum) ! for binary files
     ...
     x      /,1x,'trajectory file binary format key    ',3x,i10)")
     x      nstraj,istraj,keytrj,keybin
...
!          toler = toler/4.8d0
        ...
        elseif(record(1:4).eq.'aspc') then
          ...
          sor_omega = 6.0d0/11.0d0
          aspc_k_param = 4
          aspc_iter_nstep = 6 ! does iteration for first 6 steps
     ...
     x    '/,1X,'fictious mass of dipole   ',3x,1p,e12.4)")dipmas
        ...
        elseif(record(1:5).eq.'dtemp') then
          ...
          ldpts = .true.
          if (idnode.eq.0)
     x    write(nrite,"(/,1X,'dipole couple to Berendsen thermostat 
     x    '/,1X,'dipole temperature   ',3x,1p,e12.4)")diptmp

        elseif(record(1:10).eq.'dberendsen') then
     ...
     x    write(nrite,*)'all dipoles are smeared!'
        ...
        elseif(record(1:3).eq.'ttm') then
          ...
          lttm=.true.
          if (idnode.eq.0) then
c            if (.not.lshake) write(nrite,*)'TTM2-R water model is used!'
c            if (lshake) write(nrite,*)'TTM2-F water model is used!'
       ...
       elseif(record(1:6).eq.'thlion') then  ! Thole damping for ion-ion.

          athole_ion = dblstr(record,80,idum)
          if (idnode.eq.0) then
             write(nrite,
     x  "(/,1x,'Thole attenuating factor ion-ion',3x,1p,e12.4)")
     x       athole_ion
          end if


       elseif(record(1:6).eq.'ithole') then  ! Number of ions.

          ithole = intstr(record,80,idum)
          if (idnode.eq.0) write(nrite,
     x  "(/,1x,'Thole damping for ions',3x,i10)") ithole

       elseif(record(1:6).eq.'n_ions') then  ! Number of ions.

          n_ions = intstr(record,80,idum)
          if (idnode.eq.0) write(nrite,
     x  "(/,1x,'Number of ions',3x,i10)") n_ions
     ...
     x  "(1x,'TTM4 charge-dipole smearing factor ',3x,1p,e12.4)")ascd
       ...
       elseif(record(1:6).eq.'vmbpol') then
...
c Thole damping for MB-pol water.
          athole = 0.055d0 ! non-bonded dipole-dipole
          athole12 = 0.626d0 ! bonded dipole-dipole
          athole13 = 0.055d0 ! bonded dipole-dipole
          ...
          if (idnode.eq.0) then
             write(nrite,*) 'MB-pol model (with TTM polarization)'
             write(nrite,
     x  "(/,1x,'Thole attenuating factor (non-bonded) ',3x,1p,e12.4)")
     x       athole
             write(nrite,
     ...
     x  "(1x,'TTM4 charge-dipole smearing factor ',3x,1p,e12.4)")ascd
       ...
       elseif(record(1:6).eq.'vmbnrg') then
...
c Thole damping for MB-pol water.
          ...
          if (idnode.eq.0) then
             write(nrite,*) 'MB-nrg model (with TTM polarization)'
             write(nrite,
     x  "(/,1x,'Thole attenuating factor (non-bonded) ',3x,1p,e12.4)")
     x       athole
             write(nrite,
...
c mbnrg 
       ...
       elseif(record(1:6).eq.'thliw') then  ! Thole damping for ion-water. 

          athole_ionwat = dblstr(record,80,idum)
          if (idnode.eq.0) then
             write(nrite,
     x  "(/,1x,'Thole attenuating factor ion-water',3x,1p,e12.4)")
     x       athole_ionwat
          end if


       elseif(record(1:6).eq.'qtip4p') then

          lmsite = .true.
          gammattm = 0.26388d0

        elseif(record(1:8).eq.'nopolint') then ! no intra-molecular
                                               ! polarizability contrib
                                               ! from the charges

            lpolintra=.false.
          if (idnode.eq.0) then
             write(nrite,*) 
     x   'Excluding all intramolecular charges from pol calculation'
          endif
        ...
        elseif(record(1:6).eq.'cavity') then
...
c     read Ewald or HK-Ewald or SPM-Ewald sum parameters
          ...
          call cal_field(record1,nofields,countfields,starts,ends)
          tmprecord=record1(starts(2):ends(2))
...
c     retreive cell vectors
              ...
              do i=1,9,3
                ...
                call cal_field(record1,nofields,countfields,starts,ends)
                 ...
                 tmprecord=record1(starts(1):ends(1))
                 read(tmprecord,*) cell(i)
                 tmprecord=record1(starts(2):ends(2))
                 read(tmprecord,*) cell(i+1)
                 tmprecord=record1(starts(3):ends(3))
...
#ifdef IPI 
        elseif(record(1:3) .eq. 'ipi' )then  
           serveraddr = record(5:len(record)) 
#endif 


! PIMD/CMD

       elseif(record(1:4).eq.'pimd') then

          cmd_run = .false.

       elseif(record(1:3).eq.'cmd') then

          cmd_run = .true.

       elseif(record(1:5).eq.'rstrt') then

          restart_run = .true.

       elseif(record(1:6).eq.'rstdip') then

          restart_dipole = .true.

       elseif(record(1:6).eq.'rstcmd') then

          restart_cmd = .true.

       elseif(record(1:6).eq.'cmdnvt') then

          cmd_nve = .false.

       elseif(record(1:6).eq.'setvel') then

          set_vel = .true. 

       elseif(record(1:5).eq.'trpmd') then

          trpmd_run = .true.

       elseif(record(1:6).eq.'w_bead') then

          w_bead = .true.

       elseif(record(1:5).eq.'nbead') then

          nbead = intstr(record,80,idum)

       elseif(record(1:5).eq.'iseed') then

          iseed = intstr(record,80,idum)

          iseed = -abs(iseed)
       
       elseif(record(1:6).eq.'gamcmd') then

          gamcmd = dblstr(record,80,idum)

       elseif(record(1:6).eq.'fac_nm') then

          fac_nm = dblstr(record,80,idum)

       elseif(record(1:6).eq.'nchain') then

          nchain = intstr(record,80,idum)

       elseif(record(1:10).eq.'hod_in_d2o') then

          hod_in_d2o = .true.

       elseif(record(1:10).eq.'hod_in_h2o') then

          hod_in_h2o = .true.

       elseif(record(1:10).eq.'h2o_in_d2o') then

          h2o_in_d2o = .true.

       elseif(record(1:10).eq.'d2o_in_h2o') then

          d2o_in_h2o = .true.

       elseif(record(1:5).eq.'n_hod') then

          nmol_mix = intstr(record,80,idum)

       elseif(record(1:5).eq.'n_h2o') then

          nmol_mix = intstr(record(6:),80,idum)

       elseif(record(1:5).eq.'n_d2o') then

          nmol_mix = intstr(record(6:),80,idum)

       elseif(record(1:12).eq.'molmix_nskip') then

          molmix_nskip = intstr(record,80,idum)
! Langevin
       elseif(record(1:14).eq.'langevin_omega') then

          omega0 = dblstr(record,80,idum)

       elseif(record(1:19).eq.'langevin_cell_omega') then

          omega_cell = dblstr(record,80,idum)

       elseif(record(1:24).eq.'langevin_cell_relax_time') then

          tau_cell = dblstr(record,80,idum)

       elseif(record(1:13).eq.'langevin_type') then

          LangThermoType = intstr(record,80,idum)

       elseif(record(1:8).eq.'gle_nums') then

          numS_gle = intstr(record,80,idum)

       elseif(record(1:10).eq.'no_fix_com') then

          fix_com = .false. 

       elseif(record(1:9).eq.'gle_aunit') then

          gle_Aunit(1:71) = record(10:80)
          call strip(gle_Aunit,71)

       elseif(record(1:9).eq.'gle_cunit') then

          gle_Cunit(1:71) = record(10:80)
          call strip(gle_Cunit,71)

       elseif(record(1:9).eq.'gle_afile') then

          gle_Afile(1:71) = record(10:80)
          call strip(gle_Afile,71)

       elseif(record(1:9).eq.'gle_cfile') then

          gle_Cfile(1:71) = record(10:80)
          call strip(gle_Cfile,71)

! CV
       elseif(record(1:5).eq.'cv_ql') then

          ! assuming both Q4/Q6 for now
          ! customize options later
          CV_Ql = .true.
          nColVar = 2

       elseif(record(1:7).eq.'rmin_ql') then

          rmin_ql = dblstr(record,80,idum)

       elseif(record(1:7).eq.'rmax_ql') then

          rmax_ql = dblstr(record,80,idum)

       elseif(record(1:12).eq.'nlist_cutoff') then

          nlist_cutoff = dblstr(record,80,idum)

       elseif(record(1:9).eq.'qfour_pos') then

          ev_pos(1) = dblstr(record,80,idum)

       elseif(record(1:8).eq.'qsix_pos') then

          ev_pos(2) = dblstr(record,80,idum)

       elseif(record(1:11).eq.'qfour_kappa') then

          ev_kappa(1) = dblstr(record,80,idum)

       elseif(record(1:10).eq.'qsix_kappa') then

          ev_kappa(2) = dblstr(record,80,idum)
...
c     check thole smearing function
      ...
      if (lthole) then
        if (nthole.ne.1.and.nthole.ne.3.and.nthole.ne.4) then
           write(nrite,*)
           write(nrite,*)
     x     ' ** Error ** : Thole smearing function is not specified!'
           kill = .true.
        endif
      endif
c     
